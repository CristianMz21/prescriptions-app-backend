...
describe('UsersService', () => {
  ...
  describe('create', () => {
    it('should hash password and create a user', async () => {
      const dto = { email: 'test@clinic.com', password: 'password', role: Role.PATIENT };
  ...
  describe('findByEmail', () => {
    it('should find user by email', async () => {
      ...
      expect(prismaService.user.findUnique).toHaveBeenCalledWith({ where: { email: 'test@clinic.com' } });
  ...
  describe('findById', () => {
    it('should find user by id and return UserEntity', async () => {
      ...
      expect(prismaService.user.findUnique).toHaveBeenCalledWith({ where: { id: 'user-1' } });
  ...
  describe('findAllByRole', () => {
    it('should find all users by role', async () => {
      ...
      expect(prismaService.user.findMany).toHaveBeenCalledWith({ where: { role: Role.PATIENT } });
